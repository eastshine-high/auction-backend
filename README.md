...
위의 주문 에그리것을 아래와 같이 QueryDsl을 이용하여 조회합니다. N + 1 문제를 방지하기 위해 `@OneToMany` 관계는 모두 `fetchJoin` 을 사용하여 조회하였습니다.
...
위에서 작성한 Querydsl의 조회 문장도 수정합니다. `leftJoin` 문 하나를 제거하였습니다.
...
수행된 SQL을 확인해 보면 Join 처리하지 않은 부분은 `IN` 절로 처리되어 N+1 문제 발생 없이 조회가 되는 것을 확인할 수 있습니다.
